...
export default function SubProcessNode({ id, data }: NodeProps) {
  const label = data.label as string;
  const { editing, value, setValue, inputRef, startEditing, commit, onKeyDown } =
    useNodeLabelEdit(id, label);

  return (
    <div className={styles.subProcess} onDoubleClick={startEditing}>
      {editing ? (
        <input
          ref={inputRef}
          value={value}
      ...
      <div className={styles.subProcessMarker}>+</div>
